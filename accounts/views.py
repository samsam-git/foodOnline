...
def registerVendor(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid() and v_form.is_valid():
            ...
            messages.success(
                request, "Your account has been registered successfully. please wait for the approvale.")
            return redirect('login')
        else:
            print('Invalid Form')
            print(form.errors)
...
def activate(request, uidb64, token):
    ...
    if user is not None and default.token.generater.check_token(user, token):
        user.is_active = True
        user.save()
...
def forgot_password(request):
    ...
    return render(request, 'accounts/forgot_pasword.html')
